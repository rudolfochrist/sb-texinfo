...
(defun write-texinfo-header (title filename)
  (format *texinfo-output* "
...
(defun write-chapter/dictionary (docs &optional exclude-node)
  (unless exclude-node
    (write-line "
@node Dictionary
@comment  node-name,  next,  previous,  up
...
(defun generate-includes (directory packages &key base-package flatten extra-symbols (write-backmatter-p t))
  ...
  (handler-bind ((warning #'muffle-warning))
    (let ((directory (merge-pathnames (pathname directory)))
      ...
      (with-texinfo-file (merge-pathnames "sb-texinfo.texinfo" directory)
        ...
        (write-packageish-macro nil "pkg"))
...
(defun relative-includes (pathname)
  "Includes directory path relative to PATHNAME."
  (merge-pathnames "include/"
                   (make-pathname :defaults (merge-pathnames pathname)
                                  :name nil
                                  :type nil)))


(defun document-packages (packages title
                          &key
                            output-file
                            (standalone t)
                            (write-backmatter t)
...
for a manually maintained Texinfo manual."
  ...
  (handler-bind ((warning #'muffle-warning))
    (let* ((filename (or output-file (make-pathname :name (string-downcase title) :type "texi")))
           (*base-package* (first packages)))
      (with-texinfo-file filename
        (when standalone
          (write-texinfo-header title (pathname-name filename)))
